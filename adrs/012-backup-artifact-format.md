...
## Amendment 1 — signing tool: `ssh-keygen -Y` replaces cosign local-keypair (2026-07-05)

Discovered during implementation (#347), the same day this ADR was accepted: **cosign v3** (current Homebrew release, v3.1.1) deprecates `--tlog-upload` and `--output-signature`, defaults to a new bundle format, and routes offline key-based signing through a `--signing-config` file — the stable v2 detached-signature flow this ADR's sub-decision assumed no longer exists on current installs, and the deprecation messaging signals further change. A disaster-recovery verifier must behave identically on whatever tool version a host has **years** after the backup was written; a CLI that breaks its offline signing surface within months fails that requirement regardless of its other merits.

**Decision: the backup path signs with `ssh-keygen -Y sign` and verifies with `ssh-keygen -Y verify`** against an `allowed_signers` file — the alternative this ADR already named as acceptable. A dedicated passwordless ed25519 key (`backup_signing_key`) and `allowed_signers` (principal `expertise-backup`) are generated by `backup-init` in `~/.config/expertise-api/backup/`; `allowed_signers` is the trust root. The artifact layout is unchanged (`manifest.json.sig` is now an OpenSSH signature). OpenSSH ships on every supported OS, the `-Y` interface has been stable since OpenSSH 8.1 (2019), signing/verification are offline and non-interactive by construction, and `backup-init` no longer needs any tool outside apt/brew defaults (age + jq) — a strict improvement for the bootstrap-UX requirement.

**Scope**: backup artifacts only. The release pipeline's keyless-OIDC cosign ceremony (ADR-011) is a CI context where cosign's versioning is pinned and network/Rekor are available — it is unaffected and remains cosign. The original sub-decision's "one signing tool across releases and backups" benefit is forfeited; the corresponding "second cosign mode" drawback in Consequences disappears with it.
