...
- **Bad**, because response-stream interception (even via `IEndpointFilter` + `MemoryStream` swap) is sensitive to interaction with `IResponseHygiene` (ADR-008). The idempotency filter sits as the *outermost* endpoint filter so it buffers the already-hygienized bytes; an integration test asserts byte-equality on replay including the deterministic-on-replay hygiene nonce.
