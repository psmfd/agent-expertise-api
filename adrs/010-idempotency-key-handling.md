...
**Decision.** The truncation marker uses HTTP `Warning` code **`199`** ("Miscellaneous warning"), not `299` ("Miscellaneous persistent warning") as the original ADR text described. RFC 9111 §5.5 (which obsoleted RFC 7234 in June 2022) preserves the 1xx/2xx persistence distinction inherited from RFC 7234: `2xx` warnings persist after a response is validated and reused by an intermediary cache; `1xx` warnings are stripped on revalidation. Each idempotent-replay response is a fresh server-generated response (the replay outcome is not cached by intermediaries — `Cache-Control: no-store` applies), so the persistence semantics of `299` add no value and the simpler `199` is appropriate. Note that RFC 9111 §5.5 also marks the entire `Warning` header field as obsolete ("senders SHOULD NOT generate"); current clients still parse and surface it correctly, so we retain it for the existing soft-warn rollout window. If/when we replace this marker, the natural successor is a custom `Idempotency-Body-Truncated: true` response header — tracked as a follow-up to this amendment.
