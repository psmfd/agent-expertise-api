...
#     (mode 600) when absent — idempotent, never rotated, never echoed.
#     New installs get Integrity__HmacKeyFile wired into the secrets.env
#     stub; existing installs keep their secrets.env untouched and are
#     told exactly what line to add (see the WARN output and
#     docs/operations/integrity-verification-runbook.md).
#   * A daily `verify` schedule is installed next to the main service
#     (systemd timer on Linux, launchd StartCalendarInterval on macOS).
#     --no-verify-timer skips it; schedule manually per the runbook.
#
...
# v3 adds the active Integrity__HmacKeyFile line (ADR-020 keyed integrity
# hash). Existing v1/v2 files keep working — the API treats a missing key as
# soft-require (unkeyed legacy hashes) until the operator adds the line.
...
VERIFY_TIMER=1                # install the daily ADR-020 verify schedule; --no-verify-timer opts out

usage() { sed -n '2,97p' "$0" | sed 's/^# \{0,1\}//'; }
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --no-verify-timer)   VERIFY_TIMER=0; shift ;;
    --help|-h)           usage; exit 0 ;;
...
# ADR-020 keyed integrity hash — generated by ensure_integrity_key(), wired
# into the secrets.env stub as Integrity__HmacKeyFile on new installs.
INTEGRITY_KEY_FILE="${CONFIG_DIR}/integrity-hmac.key"
...
# Keyed integrity hash (ADR-020). install.sh generates this key file (mode
# 600) and never rotates it. Rotation procedure (Integrity__RetiredKeys__*,
# rehash --force): docs/operations/integrity-verification-runbook.md
Integrity__HmacKeyFile="${INTEGRITY_KEY_FILE}"
EOF
...
# ---------------------------------------------------------------------------
# Integrity HMAC key (ADR-020)
#
# Generate-if-absent, never rotate, never echo. Same umask-077 discipline as
# the secrets stub. secrets.env stays operator-owned: new installs get the
# Integrity__HmacKeyFile line via the stub above; existing installs get a
# WARN with the exact line to add plus the rehash/verify follow-ups — the
# installer never appends to an existing secrets.env (schema-migration
# invariant documented at detect_secrets_schema_version).
# ---------------------------------------------------------------------------
ensure_integrity_key() {
  if [[ -L "${INTEGRITY_KEY_FILE}" ]]; then
    err "${INTEGRITY_KEY_FILE} exists as a symlink — refusing to use"
  fi
  if [[ ! -f "${INTEGRITY_KEY_FILE}" ]]; then
    log "generating integrity HMAC key at ${INTEGRITY_KEY_FILE} (chmod 600, ADR-020)"
    local umask_old; umask_old=$(umask)
    umask 077
    if command -v openssl >/dev/null 2>&1; then
      openssl rand -base64 32 > "${INTEGRITY_KEY_FILE}.tmp"
    else
      # Fallback: 32 bytes from the kernel CSPRNG. base64 output is a single
      # line either way; the API trims trailing whitespace on load.
      head -c 32 /dev/urandom | base64 > "${INTEGRITY_KEY_FILE}.tmp"
    fi
    # Sanity: base64 of 32 bytes is 44 chars — refuse a short/empty read
    # rather than installing an unusable key the API will fail-closed on.
    if [[ "$(LC_ALL=C tr -d '[:space:]' < "${INTEGRITY_KEY_FILE}.tmp" | wc -c | tr -d ' ')" -lt 44 ]]; then
      rm -f -- "${INTEGRITY_KEY_FILE}.tmp"
      err "integrity key generation produced fewer than 32 bytes — check openssl / /dev/urandom availability"
    fi
    chmod 600 "${INTEGRITY_KEY_FILE}.tmp"
    mv -f -- "${INTEGRITY_KEY_FILE}.tmp" "${INTEGRITY_KEY_FILE}"
    umask "${umask_old}"
  else
    log "integrity HMAC key present at ${INTEGRITY_KEY_FILE} (preserved — never rotated)"
  fi

  # Existing (pre-v3) secrets.env without the wiring line: tell the operator
  # exactly what to do; do not mutate their file.
  if [[ -f "${SECRETS_FILE}" ]] \
      && ! grep -qE '^[[:space:]]*Integrity__HmacKey(File)?=' "${SECRETS_FILE}" 2>/dev/null; then
    warn "secrets.env has no Integrity__HmacKeyFile line — entries are hashed UNKEYED (legacy) until you wire the key (ADR-020)."
    warn "  1. Add to ${SECRETS_FILE}:  Integrity__HmacKeyFile=\"${INTEGRITY_KEY_FILE}\""
    warn "  2. Restart the service:     ${SCRIPT_DIR}/expertise-apictl restart"
    warn "  3. One-time rekey:          ${SCRIPT_DIR}/expertise-apictl rehash --force"
    warn "  4. Seal the first checkpoint: ${SCRIPT_DIR}/expertise-apictl verify"
    warn "  Full procedure: docs/operations/integrity-verification-runbook.md"
  fi
}
...
# Arguments are forwarded to the API binary so scheduled jobs can run CLI
# verbs (e.g. \`launch-expertise-api.sh verify\` from the ADR-020 timer)
# with the exact same env as the service. The service templates pass none.
...
install_launchd_system() {
  ...
  # Same reasoning for the ADR-020 integrity key: the wrapper points the API
  # at it via Integrity__HmacKeyFile and the read happens after the
  # privilege drop.
  if [[ -f "${INTEGRITY_KEY_FILE}" ]]; then
    chown "${service_user}" "${INTEGRITY_KEY_FILE}"
    chmod 600 "${INTEGRITY_KEY_FILE}"
  fi
...
# ---------------------------------------------------------------------------
# ADR-020 verify schedule — installed next to the main service unless
# --no-verify-timer. The scheduled job runs `wrapper verify` (the wrapper
# forwards args to the API binary), so it shares the service's exact env.
# NOT started/kickstarted at install time: the DB connection string may
# still be a placeholder; the first scheduled run (or a manual
# `expertise-apictl verify`) seals the first checkpoint.
# ---------------------------------------------------------------------------
install_verify_timer_systemd() {
  local unit_dir svc_path tmr_path
  unit_dir="${XDG_CONFIG_HOME:-${HOME}/.config}/systemd/user"
  svc_path="${unit_dir}/expertise-api-verify.service"
  tmr_path="${unit_dir}/expertise-api-verify.timer"
  mkdir -p "${unit_dir}"

  local svc_tpl="${SCRIPT_DIR}/service-templates/expertise-api-verify.service.tmpl"
  local tmr_tpl="${SCRIPT_DIR}/service-templates/expertise-api-verify.timer.tmpl"
  [[ -f "${svc_tpl}" ]] || err "missing template: ${svc_tpl}"
  [[ -f "${tmr_tpl}" ]] || err "missing template: ${tmr_tpl}"
  ...
  sed \
    ...
    -e "s|@@SECRETS_FILE@@|${SECRETS_FILE}|g" \
    -e "s|@@LOG_DIR@@|${LOG_DIR}|g" \
    "${svc_tpl}" > "${svc_path}.tmp"
  # Same CI escape hatch as the main unit (see install_systemd_user).
  if [[ "${EXPERTISE_API_RELAXED_HARDENING:-0}" = "1" ]]; then
    sed -i.bak -E '/^(ProtectSystem|ProtectHome|ProtectKernelTunables|ProtectKernelModules|ProtectControlGroups|PrivateDevices|RestrictNamespaces|LockPersonality|RestrictAddressFamilies|SystemCallArchitectures|PrivateTmp)=/d' "${svc_path}.tmp"
    rm -f -- "${svc_path}.tmp.bak"
  fi
  mv -f -- "${svc_path}.tmp" "${svc_path}"

  # The timer template carries no @@tokens@@ — install verbatim (tmp+mv for
  # the same atomicity discipline).
  cp "${tmr_tpl}" "${tmr_path}.tmp"
  mv -f -- "${tmr_path}.tmp" "${tmr_path}"
  log "verify timer: ${tmr_path} (daily, Persistent=true)"

  systemctl --user daemon-reload
  systemctl --user enable --now expertise-api-verify.timer
  log "verify timer: enabled (next run: systemctl --user list-timers expertise-api-verify.timer)"
}

install_verify_launchd() {
  local label="com.thesemicolon.expertise-api.verify"
  local plist_dir="${HOME}/Library/LaunchAgents"
  local plist_path="${plist_dir}/${label}.plist"
  mkdir -p "${plist_dir}"

  local tpl="${SCRIPT_DIR}/service-templates/expertise-api-verify.plist.tmpl"
  [[ -f "${tpl}" ]] || err "missing template: ${tpl}"
  ...
  sed \
    ...
    "${tpl}" > "${plist_path}.tmp"
  mv -f -- "${plist_path}.tmp" "${plist_path}"
  log "verify schedule plist: ${plist_path}"

  local domain
  domain="gui/$(id -u)"
  launchctl bootout "${domain}/${label}" 2>/dev/null || true
  launchctl bootstrap "${domain}" "${plist_path}"
  # Disabled-override guard — same LaunchDatabase caveat as the main
  # LaunchAgent (#286/#301): only `enable` when actually listed as disabled.
  if launchctl print-disabled "${domain}" 2>/dev/null \
      | grep -F "\"${label}\"" | grep -qw disabled; then
    launchctl enable "${domain}/${label}"
    log "launchd: cleared disabled-override for ${label}"
  fi
  # NO kickstart — RunAtLoad=false; the job fires on its calendar schedule.
  log "verify schedule: bootstrapped (daily 03:17)"
}

install_verify_launchd_system() {
  local label="com.thesemicolon.expertise-api.verify"
  local plist_dir="/Library/LaunchDaemons"
  local plist_path="${plist_dir}/${label}.plist"

  local tpl="${SCRIPT_DIR}/service-templates/expertise-api-verify-daemon.plist.tmpl"
  [[ -f "${tpl}" ]] || err "missing template: ${tpl}"

  # install_launchd_system already resolved + validated the service user;
  # re-resolve here so this function stands alone (order-independence).
  ...
  chmod 644 "${plist_path}.tmp"
  mv -f -- "${plist_path}.tmp" "${plist_path}"
  log "verify schedule daemon plist: ${plist_path}"
  ...
  # NO kickstart — RunAtLoad=false; the job fires on its calendar schedule.
  log "verify schedule: bootstrapped (daily 03:17, runs as ${service_user})"
}
...
install_service() {
  case "${OS}" in
    linux|wsl)
      ...
      install_systemd_user
      if (( VERIFY_TIMER == 1 )); then
        install_verify_timer_systemd
      else
        log "verify timer: skipped (--no-verify-timer) — schedule manually per docs/operations/integrity-verification-runbook.md"
      fi ;;
    macos)
      if [[ "${INSTALL_SCOPE}" == "system" ]]; then
        install_launchd_system
        if (( VERIFY_TIMER == 1 )); then
          ...
          log "verify schedule: skipped (--no-verify-timer) — schedule manually per docs/operations/integrity-verification-runbook.md"
        fi
      else
        install_launchd
        if (( VERIFY_TIMER == 1 )); then
