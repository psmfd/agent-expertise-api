...
# ADR-020 keyed integrity hash — generated by Confirm-IntegrityKey, wired to
# the service via the Environment REG_MULTI_SZ (Integrity__HmacKeyFile).
$IntegrityKeyFile = Join-Path $ConfigDir 'integrity-hmac.key'
...
function Confirm-IntegrityKey {
    # Generate-if-absent, never rotate, never echo — parity with
    # install.sh ensure_integrity_key(). 32 CSPRNG bytes, base64, ACL
    # restricted to the same principals as secrets.env.
    if (Test-Path $IntegrityKeyFile) {
        Write-Log "integrity HMAC key present at $IntegrityKeyFile (preserved - never rotated)"
        return
    }
    Write-Log "generating integrity HMAC key at $IntegrityKeyFile (ADR-020)"
    $bytes = [byte[]]::new(32)
    [System.Security.Cryptography.RandomNumberGenerator]::Fill($bytes)
    Set-Content -Path $IntegrityKeyFile -Value ([Convert]::ToBase64String($bytes)) -Encoding ascii -NoNewline
    icacls $IntegrityKeyFile /inheritance:r | Out-Null
    icacls $IntegrityKeyFile /grant:r 'Administrators:R' "NT SERVICE\$ServiceName`:R" "SYSTEM:R" | Out-Null
}
...
function Install-WindowsService {
    ...
    $envValues = @(
        ...
        # ADR-020 keyed integrity hash. After the FIRST install that adds this,
        # run the one-time rekey (rehash --force) and schedule verify — see
        # docs/operations/integrity-verification-runbook.md (Windows: schtasks).
        "Integrity__HmacKeyFile=$IntegrityKeyFile"
    )
