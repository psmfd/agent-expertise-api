...
- **Password never echoed** to stdout/stderr/logs. Generated in a
  guarded subshell, passed to `psql` via `-v pw=:'pw'` parameter
  binding (never argv — no `ps` / `/proc/*/cmdline` leak), written
  to `secrets.env` via atomic `mktemp` + rename.
...
Linux dependency bootstrap (Debian/Ubuntu, Fedora/RHEL) and the ADR
for SDK-on-host vs pre-published artifact tracking are out of scope
for this PR; see issues **#246**, **#247**, and **#245** respectively.
The current macOS path **installs the .NET SDK** rather than the
runtime alone because `scripts/install.sh` runs `dotnet publish` on
the install host (the runtime-only framing was a brief inconsistency
surfaced by the PR C pre-design review; #245 owns the long-term
resolution).
