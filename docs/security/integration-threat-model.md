...
### Part D aggregator up-sync note (ADR-013)

Up-sync introduces the first server-to-server write path: a spoke instance POSTs entries to a hub's `/expertise/batch` as an OIDC `client_credentials` principal. The threat is knowledge-supply-chain poisoning (OWASP ASI04/ASI06 — a compromised or malicious spoke seeding content that other teams' agents will consume). The control is deliberately NOT sync-specific code: the spoke's hub credential is provisioned with `expertise.write.draft` only, so ADR-003's scope semantics force every synced entry to land as `Draft` in the spoke's own hub-side tenant, invisible to every other tenant until the hub curator independently approves it. Supporting posture: hub-side tenancy is derived from the token (`TenantSource` mapping — payload tenant never trusted); `OriginInstanceId` is server-derived from the authenticated client id (`Sync:KnownInstances`), so a payload cannot self-certify its origin; the body-supplied `OriginAuthorPrincipal` is informational only, length-capped at write and hygienized as user-supplied free text on read (C7); sync traffic resolves to `ActorClass = Service` (C6), keeping it audit-distinguishable; and the blast radius of a spamming spoke is its own tenant's draft queue, bounded by the existing `expertise-write` rate limit.
