...
**Last updated:** 2026-05-22 (PRs A+B+C1 merged; ADR-011 merged; D1+D2+D2.1 merged; D3 in flight)
...
| D3 | install.sh `--from-release` opt-in (cosign verify + bsdtar extract + downgrade defense + runtime semver tighten) | #249 | ☐ in flight — branch `feat/install-from-release-cosign-verify`, pre-PR fan-out folded | Load-bearing PR. Ships `scripts/verify-release.sh` (sanctioned cosign-verify entrypoint), `scripts/lib/release-consumer.sh` (fetch/verify/inspect/extract/markers), install.sh wiring (`--from-release` / `--from-source` / `--version` / `--allow-downgrade` / `--accept-republished-version` / `--skip-release-api-crosscheck`), and `tests/install/test-release-tarball-flow.sh` (39 assertions). D2.1 (#254, `61fc24a`) hardened generate-manifest to accept frameworks-array runtimeconfig shape as a D3 precondition. Pre-PR fan-out (shell + security + checkmarx): shell-expert NEEDS_CHANGES (2 HIGH + 4 MED + 5 LOW), security-review-expert PASS_WITH_WARNINGS (3 LOW), checkmarx-expert PRECONDITION_FAILURE (cx CLI unavailable; Bash not a first-class SAST target). All HIGH + applicable MED/LOW folded: NUL-delimited safe inspector traversal (newline-bearing names refused), Unicode-aware case-fold collision detection, prerelease appVersion refused for --from-release (full SemVer §11 comparator deferred to #257), quarantine cleanup extended to install.sh cleanup(), --from-release/--from-source mutual exclusion, cosign --json case-insensitive parse, verify-release.sh re-source guard, curl --max-redirs pinned, mktemp portable template form. Two follow-up flags deferred to #255 (`--tarball-url` mirror) and #256 (`--allow-offline-verify`); SemVer §11 comparator deferred to #257. |
