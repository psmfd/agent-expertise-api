...
| `IntegrityHash` | `string?` | Canonical content hash over `{tenant, title, body, entryType, severity}`. Keyed format `{keyId}:{hex}` = HMAC-SHA256 under `Integrity:HmacKey(File)` (ADR-020, unforgeable by a DB-only writer); bare 64-hex = legacy unkeyed SHA-256 (still verifiable, counted by `verify`). The key is hard-required outside Development since #490 (ADR-020 Amendment 1); `Integrity:RequireKey=false` is the rollback overlay. Backfilled by `rehash`; rekeyed by `rehash --force`. |
